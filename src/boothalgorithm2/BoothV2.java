...
public class BoothV2 {
    ...
    public BoothV2(int bit, String multiplicandInput, String multiplierInput) {
        this.bit = bit;
...
//    }
    ...
    public void writeArr(int type, int startIndex) {
        //currLine.append("Multiplier     => ");
        if (type == 1) {
            currLine.append("Add Operation  => ");
        } else if (type == 2) {
            currLine.append("Sub Operation  => ");
        } else {
    ...
    public void startOperation() {
        ...
        for (int i = multiplier.size() - 1; i > 0; i--) {
            ...
            if (preBit == 0 && postBit == 1) {
                writeArr(1, operateStartIndex - bit + 1);
                addOperation(multiplicand);
            } else if (preBit == 1 && postBit == 0) {
                writeArr(2, operateStartIndex - bit + 1);
                addOperation(multiplicandComplement);
            } else {
                writeArr(3, operateStartIndex - bit + 1);
            }
            shiftLeft();
            printLine();
        }
